...
void *infinite(void *arg) {
    printf("In thread having infinite while loop\n");
    while(running);
    return (void *)128;
}
...
void* threadtwo(void *args) {
    mthread_t tid;
    mthread_create(&tid, NULL, threadone, NULL);
...
int main(int argc, char **argv) {
    ...
    printf("1] Create thread inside another thread\n");
    {
        count = 0;
        mthread_t tid;
        MCHECK(mthread_create(&tid, NULL, threadtwo, NULL));
        MCHECK(mthread_join(tid, NULL));
        if (count == 2) {
            printf("TEST PASSED\n\n");
        }
    ...
    printf("2] Scheduling between three threads\n");
    {
        mthread_t tid[3];
        for(int i = 0; i < 3; i++) {
            MCHECK(mthread_create(&tid[i], NULL, keep_going, NULL));
        }
        for(int i = 0; i < 3; i++) {
            MCHECK(mthread_join(tid[i], NULL));
        }
